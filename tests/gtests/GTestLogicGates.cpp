...
/ New tests for NOT gate
TEST(NOTConstructorTest, test005) {
    // create an object
    NOT not1(1);

    // check that the object is constructed correctly
    unsigned short int in1 = not1.in1();
    ASSERT_EQ(in1, 0);
    unsigned short int out = not1.out();
    ASSERT_EQ(1, out);  // NOT gate should invert 0 to 1 by default
}

// New tests for XOR gate
TEST(XORConstructorTest, test006) {
    // create an object
    XOR xor1(1);

    // check that the object is constructed correctly
    unsigned short int in1 = xor1.in1();
    ASSERT_EQ(in1, 0);
    unsigned short int in2 = xor1.in2();
    ASSERT_EQ(in2, 0);
    unsigned short int out = xor1.out();
    ASSERT_EQ(0, out);
}
